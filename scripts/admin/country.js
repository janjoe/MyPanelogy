...
$(document).ready(function() {
    ...
    jQuery("#country").jqGrid({
        ...
        loadComplete : function() {
            ...
            .css({
                "background-image":"url("+imageurl+"share_12.png)", 
                "background-position":"0", 
                "color":"black"
            });
            $("#pager").find(".ui-addtosurvey-icon")
            .css({
                "background-image":"url("+imageurl+"tokens_12.png)", 
                "background-position":"0", 
                "color":"black"
            });
        },
        ...
        ondblClickRow: function(id) {
            var can_edit = $('#country').getCell(id, 'can_edit');
            if(can_edit == 'false') {
                var dialog_buttons={};
                dialog_buttons[okBtn]=function() {
                    $( this ).dialog( "close" );
...
{
                if(id && id!==lastSel) {
                    jQuery('#country').saveRow(lastSel);
                    lastSel=id;
                }
            }
            jQuery('#country').editRow(id,true);
            }
        },
        pager: "#pager",
        pgtext: pageViewTxt,
    ...
    jQuery("#country").jqGrid(
        ...
        }, //default settings for add
        {
            ...
            afterShowForm: function($form) {
                ...
                selRowId = jQuery("#country").jqGrid('getGridParam', 'selrow'),
        ...
        onClickButton:function(){
            ...
            dialog_buttons[searchBtn]=function(){
                ...
                if($('#field_1').val() == ''){
                    ...
                    jQuery("#country").jqGrid('setGridParam',{
                        mtype:'POST',
                        postData:{
                            searchcondition:searchconditions
                        },
                        url:jsonSearchUrl,
                        gridComplete: function(){
                            if(jQuery("#country").jqGrid('getGridParam', 'records') == 0) {
                                var dialog_buttons={};
                                dialog_buttons[okBtn]=function(){
                                    $( this ).dialog( "close" );
                                };
    ...
    $("#country").navButtonAdd('#pager',
    {
        ...
        onClickButton:function() {
            $.post(
                ...
                {
                    searchcondition: searchconditions,
                    searchURL: jQuery('#country').jqGrid('getGridParam', 'url')
                },
                function(data) {
                    titlemsg = data;
                    var dialog_buttons={};
                    dialog_buttons[exportBtn]=function(){
                        $.download(exportToCSVURL,{
                            searchcondition: searchconditions, 
                            attributes: $('#attributes').val().join(' ')
                        },"POST");
                        $(this).dialog("close");
                    };
                    dialog_buttons[cancelBtn]=function(){
                ...
                if(survey_id===null) {
                    ...
                    if(rows=="") { /* All in grid */
                        $.post(
                            getSearchIDs,
                            {
                                searchcondition: jQuery('#country').jqGrid('getGridParam','url')
                            },
                            function(data) {
    ...
    function editModifier(id, subgrid_id, method) {
        ...
        var participant_id = $("#country_"+parid[0]+"_t").getCell(id,'participant_id');
        var lsel = parid[0];
        var can_edit = $('#country').getCell(participant_id,'can_edit');
        if(can_edit == 'false') {
            var dialog_buttons={};
            dialog_buttons[okBtn]=function(){
                $( this ).dialog( "close" );
            ...
            var att_type = $("#country_"+parid[0]+"_t").getCell(id,'atttype');
            if(att_type=="DP") { //Date
                $("#country_"+parid[0]+"_t").setColProp('attvalue',{
                    editoptions:{
            ...
            if(att_type=="DD") { //Dropdown
                var att_p_values = $("#country_"+parid[0]+"_t").getCell(id,'attpvalues');
                $("#country_"+parid[0]+"_t").setColProp('attvalue',{
                    edittype:'select',
                    editoptions:{
                        value:":;"+att_p_values
            ...
            var attap = $("#country_"+parid[0]+"_t").getCell(id,'attap');
            if(id && id!==lastSel2) { //If there was already another row open for editin save it before editing this one
                jQuery("#country_"+parid[0]+"_t").saveRow(lastSel2);
                //jQuery.fn.fmatter.rowactions('97358ea2-8227-483b-a225-5d13a522402e_50','country_97358ea2-8227-483b-a225-5d13a522402e_t','cancel',0);
                lastSel2=id;
            }
            $.fn.fmatter.rowactions(id,'country_'+parid[0]+'_t','edit',0);
            if(method=='edit') {
                jQuery("#country_"+parid[0]+"_t").jqGrid('restoreRow',id);
                jQuery("#country_"+parid[0]+"_t").restoreRow(id);
            }
            jQuery("#country_"+parid[0]+"_t").jqGrid('editRow',id,true);
            jQuery("#country_"+parid[0]+"_t").editRow(id,true);
        }
    }
