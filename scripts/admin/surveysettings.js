    ...
    $("#urlparams").jqGrid({
        ...
        gridComplete: function(){
            ...
            for(var i=0;i < ids.length;i++)
            {
                ...
                jQuery("#urlparams").jqGrid('setRowData',ids[i],{
                    act:be+de
                    });
            }
        }
...
function saveParameter()
{
    ...
    if ($("#dlgEditParameter").data('action')=='add')
    {
        ...
        jQuery("#urlparams").addRowData(sGUID, {
            act: "<image style='cursor:pointer;' src='"+imageUrl+"/edit_16.png' value='E' onclick=\"editParameter('"+sGUID+"');\" />"
    ...
    {
        jQuery("#urlparams").setRowData($("#dlgEditParameter").data('rowid'),{
            parameter:sParamname,
            targetqid:sTargetQID,
            targetsqid:sTargetSQID,
            title:$("#targetquestion option:selected").text()
