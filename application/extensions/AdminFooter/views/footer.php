<div class='footer'>
    <div style='float:left;width:110px;text-align:left;'>
        <a href='http://docs.survey-office.com' target="_blank"><img alt='SurveyOffice - <?php eT("Online Manual"); ?>' title='SurveyOffice - <?php eT("Online manual"); ?>' src='<?php echo Yii::app()->getConfig('adminimageurl'); ?>docs.png' /></a>
    </div>
    <div style='float:right;'>
        <a href='http://survey.braininfotech.com/limesurvey' target="_blank">
            <img alt='SurveyOffice' width="100" title='SurveyOffice' src='<?php echo Yii::app()->getConfig('adminimageurl'); ?>Logo.png'/></a>
    </div>
</div>
