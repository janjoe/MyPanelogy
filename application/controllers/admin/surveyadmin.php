...
class SurveyAdmin extends Survey_Common_Action {
    ...
    public function regenquestioncodes($iSurveyID, $sSubAction) {
        if (Permission::model()->hasSurveyPermission($iSurveyID, 'surveycontent', 'update')) {
            ...
            foreach ($gresult->readAll() as $grow) {
                $grows[] = $grow;
            } // Get table output into array
            foreach ($grows as $grow) {
                //Go through all the questions
                if ($sSubAction == 'bygroup' && (!isset($group_number) || $group_number != $grow['gid'])) { //If we're doing this by group, restart the numbering when the group number changes
                    $question_number = 1;
    ...
    public function getSurveys_json() {
        ...
        foreach ($surveys as $rows) {
            if (!isset($rows->owner->attributes))
                $aOwner = array('users_name' => $clang->gT('(None)')); else
