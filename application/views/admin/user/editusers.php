    ...
    <thead>
        ...
        <tr >
            <td style='padding:3px;'>
                ...
                <?php if ($usrhimself['parent_id'] != 0 && Permission::model()->hasGlobalPermission('Regions', 'delete')) { ?>
        ...
        for ($i = 1; $i <= count($usr_arr); $i++) {
            ...
            <tr>
                ...
                <td style='padding:3px;'>          
                    <?php if (Permission::model()->hasGlobalPermission('superadmin', 'read') || $usr['uid'] == Yii::app()->session['loginID'] || (Permission::model()->hasGlobalPermission('Regions', 'update') && $usr['parent_id'] == Yii::app()->session['loginID'])) { ?>
                        <?php echo CHtml::form(array('admin/user/sa/modifyuser'), 'post'); ?>            
                        <input type='image' src='<?php echo $imageurl; ?>edit_16.png' alt='<?php $clang->eT("Edit this user"); ?>' />
                        <input type='hidden' name='action' value='modifyuser' />
                    ...
                    if (((Permission::model()->hasGlobalPermission('superadmin', 'read') &&
                            ...
                            (Permission::model()->hasGlobalPermission('Regions', 'update') &&
...
<!--                        <input type='image' src='<?php echo $imageurl; ?>templatepermissions_small.png' alt='<?php $clang->eT("Set template permissions for this user"); ?>' />
                    ...
                    <?php }
                    if ((Permission::model()->hasGlobalPermission('superadmin', 'read') || (Permission::model()->hasGlobalPermission('Regions', 'delete') && $usr['parent_id'] == Yii::app()->session['loginID'])) && $usr['uid'] != 1) { ?>
...
<?php if (Permission::model()->hasGlobalPermission('superadmin', 'read') || Permission::model()->hasGlobalPermission('Regions', 'create')) { ?>
    <?php echo CHtml::form(array('admin/user/sa/adduser'), 'post'); ?>            
    <table class='users'><tr class='oddrow'>
            <th><?php $clang->eT("Add user:"); ?></th>
            <td style='width:20%'><input type='text' name='new_user' placeholder="User Name" /></td>
            <td style='width:20%'><input type='text' name='new_email' placeholder="Email Id" /></td>
            <td style='width:20%'><input type='text' name='new_full_name' placeholder="Full Name"  /></td><td style='width:8%'>&nbsp;</td>
