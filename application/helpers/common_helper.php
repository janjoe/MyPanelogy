...
function getQidNext($surveyid, $gid, $qid) {
    ...
    if ($iNext < count($qrows)) {
        $QidNext = $qrows[$iNext]->qid;
    } else {
        $QidNext = "";
    }
    return $QidNext;
...
function getQuestionClass($input) {
    ...
    switch ($input) {   // I think this is a bad solution to adding classes to question
...
function getNotificationList($notificationcode) {
    ...
    if (!isset($ntypeselector)) {
        $ntypeselector = "";
    }
    foreach ($ntypes as $ntcode => $ntdescription) {
        $ntypeselector .= "<option value='$ntcode'";
        if ($notificationcode == $ntcode) {
            $ntypeselector .= " selected='selected'";
        }
...
function getGroupList($gid, $surveyid) {
    ...
    if ($groupselecter) {
        if (!isset($gvexist)) {
            $groupselecter = "<option selected='selected'>" . $clang->gT("Please choose...") . "</option>\n" . $groupselecter;
        } else {
            $groupselecter .= "<option value='" . Yii::app()->getConfig('scriptname') . "?sid=$surveyid&amp;gid='>" . $clang->gT("None") . "</option>\n";
        }
    }
...
function getGroupListLang($gid, $language, $surveyid) {
    ...
    if ($groupselecter) {
        ...
        if (!isset($gvexist)) {
            ...
            $groupselecter .= "<option value='{$link}'>" . $clang->gT("None") . "</option>\n";
        }
    }
    return $groupselecter;
}

// add by gaurang brain on 2014-02-27
...
define('EMAIL_POINT_PL_EditPassword', 104);
...
function getPanelRR($panelist_id) {
    $rrper = 0;
    $statusSql = "Select max_no_study, balance_no_study, no_invited, (no_redirected+no_completed+no_disqualified+no_qfull) as tResponse from {{panel_list_master}} where panel_list_id = '" . $panelist_id . "' ";
    $rs = Yii::app()->db->createCommand($statusSql)->queryRow();
    if ($rs['no_invited'] == 0) {
        $rrper = 100;
    } else {
        $rrper = round($rrper + (($rs['tResponse'] * 100) / $rs['no_invited']), 2);
    }
...
function questiontype() {
    $sql = "SELECT id,name FROM {{profile_question_type}}";
    $uresult = Yii::app()->db->createCommand($sql)->query()->readAll();
    $userlist = CHtml::listData($uresult, 'id', 'name');
    return $userlist;
}

function getProjectStatusCount_Trueup($project_id, $status) {
    $sql = "select sum(1) as total
        from {{view_panellist_redirects}}
        where project_id = '" . $project_id . "'
        and redirect_status_id in ( '" . $status . "')";
    $uresult = Yii::app()->db->createCommand($sql)->queryRow(); //Checked
    return $uresult['total'];
}

function getVStatusCount_Trueup($project_id, $vendor_id, $status) {
    $sql = "select sum(1) as total
        from {{view_panellist_redirects}}
        where project_id = '" . $project_id . "' and
        vendor_project_id = '" . $vendor_id . "'
        and redirect_status_id in ('" . $status . "')";
    $uresult = Yii::app()->db->createCommand($sql)->queryRow(); //Checked
    $total = $uresult['total'];
    if ($total == '') {
...
function messagePanellist($pl_id, $status) {
//$sql = "SELECT * FROM {{email_message}} WHERE (email_to = '$pl_id' or email_from = '$pl_id') AND status = '$status' order by id desc";//19/06/2014 Remove BY Parth
    $sql = "SELECT * FROM {{email_message}} WHERE email_to = '$pl_id' AND Sender='B' AND status = '$status' order by id desc"; //19/06/2014 Add BY Parth
    $uresult = Yii::app()->db->createCommand($sql)->query()->readAll(); //Checked
    $userlist = array();
...
//25/06/2014 Add By Hari
function EmailMessage($Parent, $Id, $status, $ParentID="") {
    $sql = " SELECT x.*, IFNULL(pm.subject,' ') AS parent_subject, IFNULL(pm.body,' ') AS parent_body ";
    $sql .= " FROM ( ";
    $sql .= " SELECT em.* ";
    $sql .= " , (SELECT COUNT(*) FROM {{email_message}} WHERE parent=em.id) AS tot_childs ";
    $sql .= " , CASE WHEN (SELECT COUNT(*) FROM {{email_message}} WHERE parent=em.id)>0 THEN 1 ELSE 0 END AS isparent ";
    $sql .= " , CASE WHEN (SELECT COUNT(*) FROM {{email_message}} WHERE parent=em.id)>0 THEN (SELECT MAX(STATUS) FROM {{email_message}} WHERE parent=em.id) ELSE em.STATUS END AS chain_status ";
    $sql .= " FROM `{{email_message}}` em ";
    $sql .= " ) AS x LEFT OUTER JOIN {{email_message}} AS pm ON x.parent=pm.id ";
    $sql .= " WHERE (x.email_to = '" . $Id . "' OR x.email_from = '" . $Id . "') ";
    if ($Parent == "True") {
        $sql .= " AND x.parent='0' ";
    } elseif ($Parent == "False") {
        $sql .= " AND x.parent='" . $ParentID . "' ";
    } elseif ($status == "Unread" || $status == "Read") {
        $sql = " AND chain_status='" . $ReadStatus . "' ";
    }
    $sql .= " ORDER BY created_datetime DESC ";
    return $uresult = Yii::app()->db->createCommand($sql)->query()->readAll();
}

//25/06/2014 End
//Start Add by Parth & Hari 14-06-2014
function receivemessage($sender, $from_to) {
    $sql = "SELECT * FROM {{email_message}} WHERE sender='" . $sender . "' AND email_from='" . $from_to . "' ";
    $uresult = Yii::app()->db->createCommand($sql)->query()->readAll(); //Checked
    $userlist = array();
    foreach ($uresult as $srow) {
        $userlist[] = $srow;
...
function adminmessage($pl_id) {
    $sql = "SELECT * FROM {{email_message}} WHERE sender = 'B' AND email_to = '$pl_id' order by id desc";
    $uresult = Yii::app()->db->createCommand($sql)->query()->readAll(); //Checked
    $userlist = array();
    foreach ($uresult as $srow) {
        $userlist[] = $srow;
...
function get_edit_answer($que_id, $que_fieldtype, $pl_value) {
    ...
    if ($que_fieldtype == 'Radio') {
        foreach ($que_ans as $value) {
            ...
            $html .= '<input ' . $selected . ' type="radio" value="' . $value['id'] . '" name="' . $que_id . '" id="' . $value['title'] . '" required="required" />' . $value['title'];
        }
    }
    if ($que_fieldtype == 'DropDown') {
        $html .= '<select name="' . $que_id . '" id="' . $que_id . '" required="required">
        ...
        foreach ($que_ans as $value) {
            if ($value['title'] == '[[COUNTRY]]') {
                ...
                foreach ($country as $val) {
                    $selected = '';
                    if ($val['country_id'] == $pl_value) {
                        $selected = 'selected = "selected"';
                    }
                    $html .= '<option ' . $selected . ' value="' . $val['country_id'] . '">' . $val['country_name'] . '</option>';
                }
                ...
                foreach (getLanguageDataRestricted(false, Yii::app()->session['adminlang']) as $langkey => $langname) {
                    ...
                    $html .= '<option ' . $selected . ' value="' . $langkey . '">' . $langname['description'] . '</option>';
                }
            } else {
                $selected = '';
                if ($value['id'] == $pl_value) {
    ...
    if ($que_fieldtype == 'CheckBox') {
        ...
        foreach ($que_ans as $value) {
            ...
            $html .= '<input ' . $selected . ' type="checkbox" onchange="chngattribute();
" value="' . $value['id'] . '" name="' . $que_id . '[]" id="' . $value['title'] . '" required="required"/>' . $value['title'];
        }
    }
    return $html;
}

function get_answer($answer_id, $que_id) {
    $sql_ans = "SELECT * FROM {{profile_answer}} WHERE question_id='$que_id' AND IsActive = '1' ORDER BY sorder";
    $que_ans = Yii::app()->db->createCommand($sql_ans)->query()->readAll();
    foreach ($que_ans as $vl) {
        if ($vl['title'] == '[[COUNTRY]]') {
            $oRecord = Country::model()->findByPk($answer_id);
            return $oRecord['country_name'];
            //return $uresult['title'];
        } elseif ($vl['title'] == '[[LANGUAGE]]') {
            foreach (getLanguageDataRestricted(false, Yii::app()->session['adminlang']) as $langkey => $langname) {
                if ($answer_id == $langkey) {
                    return $langname['description'];
                }
            }
        } else {
            $sql = "SELECT id, title FROM {{profile_answer}} WHERE id='$answer_id' AND IsActive = '1'";
...
function get_question_categoryid($que_title) {
    $sql = "SELECT id FROM {{profile_category}} WHERE title ='$que_title' AND IsActive = 1";
    $uresult = Yii::app()->db->createCommand($sql)->queryRow();
    return $uresult['id'];
}
...
function getCountry($outputformat='fullinfoarray') {
    $clang = Yii::app()->lang;

    $uquery = "SELECT * FROM {{country_master}} ORDER BY country_name";
...
function getContactGroup($outputformat='fullinfoarray') {
    $clang = Yii::app()->lang;

    $uquery = "SELECT * FROM {{contact_group_master}} ORDER BY contact_group_id desc";
...
function getContactTypeDelete($outputformat='fullinfoarray') {
    $clang = Yii::app()->lang;

    $uquery = "SELECT * FROM {{map_company_n_types}}";
...
function GetEmailUseInArray() {
    return array('' => 'Select Email Point'
        , EMAIL_POINT_Admin_Edit_Password => 'Admin Users, Edit Password'
...
function getEmailUseInWord($id) {
    $allkeys = GetEmailUseInArray();
    if (array_key_exists($id, $allkeys)) {
        return $allkeys[$id];
    } else {
        return 'Not Found';
    }
...
function getContactTitleDelete($outputformat='fullinfoarray') {
    $clang = Yii::app()->lang;

    $uquery = "SELECT * FROM {{map_contact_n_titles}}";
...
function getContactTitle($outputformat='fullinfoarray') {
    $clang = Yii::app()->lang;

    $uquery = "SELECT * FROM {{contact_title_master}} ORDER BY contact_title_id desc";
...
function getContactType($outputformat='fullinfoarray') {
    $clang = Yii::app()->lang;

    $uquery = "SELECT * FROM {{company_type_master}} ORDER BY company_type_id desc";
...
function getCompanyDetail($company_id = null) {
    $clang = Yii::app()->lang;

    $uquery = "SELECT * FROM {{view_company}}";
    if ($company_id) {
        $uquery .= " WHERE contact_id = " . $company_id;
    }
    $uquery .= " ORDER BY contact_id DESC";
...
function projectview($project_id = null) {
    ...
    $uquery .= " order by  project_id desc ";
    $uresult = Yii::app()->db->createCommand($uquery)->query()->readAll(); //Checked
    $userlist = array();
    foreach ($uresult as $srow) {
        $userlist[] = $srow;
...
function project_vendor_view($project_id = null, $vendor_project_id=null) {
    $clang = Yii::app()->lang;

    $uquery = "SELECT * FROM {{view_project_master_vendors}} WHERE 1=1 ";
    if ($project_id) {
        $uquery .= " AND project_id = " . $project_id;
    }
    if ($vendor_project_id) {
        $uquery .= " AND vendor_project_id = " . $vendor_project_id;
    }
    $uresult = Yii::app()->db->createCommand($uquery)->query()->readAll(); //Checked
    $userlist = array();
    foreach ($uresult as $srow) {
...
function getContactDetail($contact_id = null) {
    $clang = Yii::app()->lang;

    $uquery = "SELECT * FROM {{view_contacts}}";
    if ($contact_id) {
        $uquery .= " WHERE contact_id = " . $contact_id;
    }
    $uquery .= " order by contact_id desc ";
    $uresult = Yii::app()->db->createCommand($uquery)->query()->readAll(); //Checked
    $userlist = array();
...
function getsendinghistory($project_id = 0) {
    $clang = Yii::app()->lang;

    $uquery = "Select x.* from (
select  'Send' as type, query_id, created_date as dt, count(*) as queue, 
CASE 
When reminder = 0 Then MAX(status) 
ELSE 1 
End
as status 
...
select 'Reminder' as type, query_id, send_date as dt, count(*) as queue, MAX(status) as status 
from  {{query_send_details }}
where project_id = $project_id  and reminder=1
group by send_date ) as x 
...
function getmaxsendid() {
    $quesql = "select IFNULL(max(send_id),0) as maxsendid from {{query_send_details}}";
...
function GetPanellistIDsForSend($query_id = 0, $project_id=0, $type="Send", $stack = "") { //changed on 15-05 for reminders
    $stringq = "select qstring from {{query_master}} where id = $query_id ";
    $qrydetail = Yii::app()->db->createCommand($stringq)->query()->readAll();
    $qstring = " ";
    foreach ($qrydetail as $rs) {
        $qstring = $rs['qstring'];
    }
    ...
    if ($type == "Send") {
        ...
        $quesql .= $qstring; //filter fro query
        $quesql .= " And panellist_id not in( select panellist_id from {{query_send_details}} where project_id = $project_id)"; //filter not to send originally send 
        $quesql .= " $stack";
    }
    if ($type == "Resend") {
        $quesql = "Select panellist_id from {{query_send_details}} where project_id = $project_id ";
        $quesql .= " and send = 1 and reminder = 0 and status = 1"; //filter not to send originally send 
        $quesql .= " $stack";
    }
    ...
    foreach ($qrydetail as $srow) {
        $output[] = $srow;
    }
    return $output;
}

function isquerysent($query_id = 0) {
    $quesql = "select count(*) as sum from {{query_send_details}} where query_id = " . $query_id . "";
    $qrydetail = Yii::app()->db->createCommand($quesql)->query()->readAll();
    foreach ($qrydetail as $rs) {
        return $rs['sum'];
    }
...
function getQuestionText($question_id=0) {
    $quesql = "select short_title from {{profile_question}} where id = '" . $question_id . "' ";
    $qrydetail = Yii::app()->db->createCommand($quesql)->query()->readAll();
    foreach ($qrydetail as $rs) {
        return $rs['short_title'];
    }
}
...
function my_question_id($title="") {

    $clang = Yii::app()->lang;
    $uquery = "SELECT id FROM {{profile_question}}";
    if ($title != "") {
        $uquery .= " WHERE short_title = '$title'";
    }
    $uresult = Yii::app()->db->createCommand($uquery)->query()->readAll(); //Checked
    $userlist = array();
    foreach ($uresult as $srow) {
...
function queryview($id = null, $pid=null) {
    $clang = Yii::app()->lang;

    $uquery = "SELECT * FROM {{query_master}} WHERE 1=1 ";
    if ($id)
        $uquery .= " AND id = " . $id;
    if ($pid)
        $uquery .= " AND project_id = " . $pid;

    $uquery .= " ORDER BY id DESC";
    $uresult = Yii::app()->db->createCommand($uquery)->query()->readAll(); //Checked
    $userlist = array();
    foreach ($uresult as $srow) {
        $userlist[] = $srow;
...
function rewardsview($id = null, $date = null) {
    $clang = Yii::app()->lang;

    $uquery = "SELECT * FROM {{reward_master}} WHERE IsActive = 1 ";
    if ($id) {
        $uquery .= " AND id = " . $id;
    }
    if ($date) {
        $uquery .= " AND expiration_date >= " . $date;
    }
    $uquery .= " order by id desc";
    $uresult = Yii::app()->db->createCommand($uquery)->query()->readAll(); //Checked
    $userlist = array();
...
function availablesurvey($id = null) {
    $clang = Yii::app()->lang;

    $uquery = "SELECT * FROM {{panellist_project}} WHERE status = 'A' ";
    if ($id) {
        $uquery .= " AND panellist_id = " . $id;
    }
    $uquery .= " ORDER BY created_date DESC";
    $uresult = Yii::app()->db->createCommand($uquery)->query()->readAll(); //Checked
    $userlist = array();
    foreach ($uresult as $srow) {
        $userlist[] = $srow;
...
function profilecategoryview($id = null) {
    $clang = Yii::app()->lang;

    $uquery = "SELECT * FROM {{profile_category}}";
    if ($id) {
        $uquery .= " WHERE id = " . $id;
    }
    $uquery .= " order by id desc";
...
function profilequestionview($id = null) {
    $clang = Yii::app()->lang;

    $uquery = "SELECT * FROM {{profile_question}}";
    if ($id) {
        $uquery .= " WHERE id = " . $id;
    }
    $uquery .= " order by id desc";
...
function profileanswer_view($id = null) {
    $clang = Yii::app()->lang;

    $uquery = "SELECT * FROM {{profile_answer}}";
    if ($id) {
        $uquery .= " WHERE question_id = " . $id;
    }
    $uquery .= " order by id desc";
    $uresult = Yii::app()->db->createCommand($uquery)->query()->readAll(); //Checked
    $userlist = array();
...
function profileanswerview($questionid = 0, $answerid = 0, $all= null) {
    $clang = Yii::app()->lang;

    $uquery = "select b.*, a.id as q_id, a.short_title as question_title, a.category_id as cat_id, c.title as category_title
from {{profile_category}} as c right join
{{profile_question}} as a on c.id = a.category_id left join
{{profile_answer}} as b on b.question_id=a.id ";
    if ($questionid != 0)
        $uquery.= "where a.id= $questionid";
    if ($answerid != 0)
        $uquery.= "where b.id= $answerid";
    if ($all)
        $uquery.=""; else
        $uquery.=" LIMIT 1 ";
    $uresult = Yii::app()->db->createCommand($uquery)->query()->readAll(); //Checked
...
function getPagelist() {
    ...
    $uquery = "SELECT * FROM {{cms_page_master}}";
    $uresult = Yii::app()->db->createCommand($uquery)->query()->readAll(); //Checked
    $userlist = array();
    foreach ($uresult as $srow) {
        $userlist[] = $srow;
...
function getPageDetail($page_id) {
    ...
    $uquery = "SELECT * FROM {{cms_page_master}} AS pm 
LEFT JOIN {{cms_page_content}} AS pc ON pc.page_id = pm.page_id 
WHERE pm.page_id = '$page_id' LIMIT 1";
    $uresult = Yii::app()->db->createCommand($uquery)->query()->readAll(); //Checked
    $userlist = array();
    foreach ($uresult as $srow) {
        $userlist[] = $srow;
...
function getPagContentLanguage($page_id, $page_language) {
    $clang = Yii::app()->lang;

    $uquery = "SELECT page_content FROM {{cms_page_content}} 
...
function getZoneListcountry($loadId) {
    $clang = Yii::app()->lang;

    $uquery = "select zone_id,zone_Name from {{zone_master}}
where country_id='" . $loadId . "' order by zone_Name asc";
...
function getStateListcountry($loadId) {
    $clang = Yii::app()->lang;

    $uquery = "select state_id,state_Name from {{state_master}}
where state_id='" . $loadId . "' order by state_Name asc";
...
function getEmailSubject($language, $subjectid = null) {
    $clang = Yii::app()->lang;

    $uquery = "SELECT es.email_subjectid,es.subject_text,ts.translated_subject,ts.IsActive,ts.language_code_dest
FROM {{template_email_subjects}} es
LEFT OUTER JOIN {{translation_email_subjects}} ts ON ts.email_subjectid = es.email_subjectid
AND ts.language_code_dest = '$language'";
    if ($subjectid) {
        $uquery .= " WHERE es.email_subjectid = '$subjectid'";
    }
    $uresult = Yii::app()->db->createCommand($uquery)->query()->readAll(); //Checked
    $userlist = array();
    foreach ($uresult as $srow) {
...
function getEmailBody($language, $bodyid = null) {
    $clang = Yii::app()->lang;

    $uquery = "SELECT eb.email_bodyid,eb.content_text,tb.translated_body,tb.language_code_dest,tb.IsActive
FROM {{template_email_body}} eb
LEFT OUTER JOIN {{translation_email_body}} tb ON tb.email_bodyid = eb.email_bodyid
AND tb.language_code_dest = '$language'";
    if ($bodyid) {
        $uquery .= " WHERE eb.email_bodyid = '$bodyid'";
    }
    $uresult = Yii::app()->db->createCommand($uquery)->query()->readAll(); //Checked
    $userlist = array();
    foreach ($uresult as $srow) {
...
function getEmailTemplate($templateid = null) {
    ...
    $uquery = "SELECT * FROM {{view_email_template}}";
    if ($templateid) {
        $uquery .= " WHERE template_emailid = '$templateid'";
    }
    $uresult = Yii::app()->db->createCommand($uquery)->query()->readAll(); //Checked
    $userlist = array();
    foreach ($uresult as $srow) {
...
function getCityandStateandZoneandCountry() {
    $clang = Yii::app()->lang;

    $uquery = "SELECT c.*, IFNULL(s.state_Name,'No State') AS state_Name,
IFNULL(z.zone_Name,'No zone') AS zone_Name,
IFNULL(r.country_name,'No country') AS country_name 
FROM {{city_master}} c
LEFT JOIN {{state_master}} s ON s.state_id = c.state_id 
LEFT JOIN {{zone_master}} z ON z.zone_id = s.zone_id
LEFT JOIN {{country_master}} r ON r.country_id = z.country_id
...
function getCitylistcountry($country_id) {
    $clang = Yii::app()->lang;

    $uquery = "SELECT c.*, IFNULL(s.state_Name,'No State') AS state_Name,
z.zone_id AS zone_id,r.country_id AS country_id,
IFNULL(z.zone_Name,'No zone') AS zone_Name,
IFNULL(r.country_name,'No country') AS country_name 
FROM {{city_master}} c
LEFT JOIN {{state_master}} s ON s.state_id = c.state_id 
...
LEFT JOIN {{country_master}} r ON r.country_id = z.country_id";
    if ($country_id != 0) {
        $uquery .=" WHERE r.country_id = $country_id";
    }
    $uquery .=" ORDER BY city_Name asc";
    $uresult = Yii::app()->db->createCommand($uquery)->query()->readAll(); //Checked
    $userlist = array();
    foreach ($uresult as $srow) {
        $userlist[] = $srow;
...
function getCitylistcountryUpdate($country_id) {
    $clang = Yii::app()->lang;
...
LEFT JOIN {{country_master}} r ON r.country_id = z.country_id";
    if ($country_id != 0) {
        $uquery .=" WHERE city_id = $country_id";
    }
    $uquery .=" ORDER BY city_Name asc";
    $uresult = Yii::app()->db->createCommand($uquery)->query()->readAll(); //Checked
    $userlist = array();
...
function getZoneList($outputformat='fullinfoarray') {
    $clang = Yii::app()->lang;

    $uquery = "SELECT z.zone_id,CONCAT(z.zone_Name,' - ',r.country_name) AS zone FROM {{zone_master}} z 
LEFT JOIN {{country_master}} r ON r.country_id = z.country_id ORDER BY zone_Name asc";
...
function getZone($outputformat='fullinfoarray') {
    $clang = Yii::app()->lang;

    $uquery = "SELECT * FROM {{zone_master}} ORDER BY zone_Name asc";
...
function getZoneandCountry($outputformat='fullinfoarray') {
    $clang = Yii::app()->lang;

    $uquery = "SELECT z.*,r.country_name FROM {{zone_master}} z
LEFT JOIN {{country_master}} r ON r.country_id = z.country_id
...
function getStateandZoneandCountry($id='') {
    $clang = Yii::app()->lang;

    $uquery = "SELECT s.*,IFNULL(z.zone_Name,'No zone') AS zone_Name,z.country_id AS country_id,
IFNULL(r.country_name,'No country') AS country_name FROM {{state_master}} s 
LEFT JOIN {{zone_master}} z ON z.zone_id = s.zone_id
LEFT JOIN {{country_master}} r ON r.country_id = z.country_id";
    if ($id) {
        $uquery .= " WHERE state_id = " . $id;
    }
    $uquery .= " ORDER BY state_Name asc";
    $uresult = Yii::app()->db->createCommand($uquery)->query()->readAll(); //Checked
    $userlist = array();
    foreach ($uresult as $srow) {
        $userlist[] = $srow;
...
function fill_quota_buffer($quotabuffer = 0, $Quota_Val) {
    $quota = $Quota_Val;
    $percent10 = $Quota_Val + ($Quota_Val * 0.1);
    $percent50 = $Quota_Val + ($Quota_Val * 0.5);
    $percent100 = $Quota_Val * 2;
    $qbuffer = "infinite";
    switch ($quotabuffer) {
        case $quota : $qbuffer = "percent0";
            break;
        case $percent10 : $qbuffer = "percent10";
            break;
        case $percent50 : $qbuffer = "percent50";
            break;
        case $percent100 : $qbuffer = "percent100";
            break;
        case 0 : $qbuffer = "infinite";
            break;
    }
    $quota_array = array("infinite" => "No Max", "percent0" => "0% (" . $quota . ")", "percent10" => "10% (" . $percent10 . ")", "percent50" => "50% (" . $percent50 . ")", "percent100" => "100% (" . $percent100 . ")");
    $display_q = '';

    foreach ($quota_array as $s_key => $s_value) {
        if ($s_key == $qbuffer) {
            $sc_selected = 'selected';
        } else {
            $sc_selected = '';
        }
        $display_q .= '<option value="' . $s_key . '" ' . $sc_selected . '>' . $s_value . '</option>';
    }
...
function getVendor($id) {
    if ($id == "" OR $id == 0) {
        ...
        $query = 'SELECT v.company_name AS company_name FROM {{view_company}} c,{{view_project_master_vendors}} v  WHERE c.contact_id=v.vendor_id  AND v.vendor_project_id =' . $id;
        $uresult = Yii::app()->db->createCommand($query)->queryRow();
        $userlist = array();
        foreach ($uresult as $srow) {
            $userlist[] = $srow;
        }
        return $uresult['company_name'];
    }
}

// end by gaurang brain on 2014-02-27
...
//Find out the question type
        ...
        switch ($this_type) {
            ...
            case "P":
                switch ($sValue) {
                    case "Y": $this_answer = $oLanguage->gT("Yes");
                        break;
                }
                break;
            case "Y":
                switch ($sValue) {
                    case "Y": $this_answer = $oLanguage->gT("Yes");
                        break;
                    case "N": $this_answer = $oLanguage->gT("No");
                        break;
            ...
            case "G":
                switch ($sValue) {
                    case "M": $this_answer = $oLanguage->gT("Male");
                        break;
                    case "F": $this_answer = $oLanguage->gT("Female");
                        break;
                    default: $this_answer = $oLanguage->gT("No answer");
                }
                break;
            case "C":
                switch ($sValue) {
                    ...
                    case "N": $this_answer = $oLanguage->gT("No");
                        break;
                    case "U": $this_answer = $oLanguage->gT("Uncertain");
                        break;
                }
                break;
            case "E":
                switch ($sValue) {
                    case "I": $this_answer = $oLanguage->gT("Increase");
                        break;
                    case "D": $this_answer = $oLanguage->gT("Decrease");
                        break;
                    case "S": $this_answer = $oLanguage->gT("Same");
                        break;
                }
                break;
            case "F":
...
// Attachment is either an array with filename and attachment name.
            ...
            } else { // Or a string with the filename.
    ...
    if (isset($attributes['array_filter']) && Yii::app()->session['fieldarray']) {
        ...
        foreach (Yii::app()->session['fieldarray'] as $fields) {
            if ($fields[2] == $val) {
                ...
                if ($other == "Y") {
                    if (Yii::app()->session[$fields[1] . 'other'] && Yii::app()->session[$fields[1] . 'other'] != "") {
...
function incompleteAnsFilterState() {
    ...
    if ($letsfilter != '') { // use the read value if not empty
        Yii::app()->session['incompleteanswers'] = $letsfilter;
    } elseif (empty(Yii::app()->session['incompleteanswers'])) { // sets default variable value from config file
    ...
    } else { // last resort is to prevent filtering
        return false;
    }
}
...
function fixCKeditorText($str) {
    ...
    if (trim($str) == "&nbsp;" || trim($str) == '') { // chrome adds a single &nbsp; element to empty fckeditor fields
        $str = "";
    }
...
function showJavaScript($sContent) {
    $text = preg_replace_callback('@<script[^>]*?>.*?</script>@si', create_function(
                    ...
                    '$matches', 'return htmlspecialchars($matches[0]);'
            ), $sContent);
    return $text;
}
...
function getNumericalFormat($lang = 'en', $integer = false, $negative = true) {
    ...
    if ($integer === false)
        $goodchars .= ".";    //Todo, add localisation
    if ($negative === true)
        $goodchars .= "-";    //Todo, check databases
    return $goodchars;
}
...
function checkMoveQuestionConstraintsForConditions($sid, $qid, $newgid="all") {
    ...
    foreach ($condresult->readAll() as $condrow) {
        ...
        if ($newgid != "all") { // Get only constraints when trying to move to this group
            if ($newgorder < $targetgorder) {
                $resarray['notAbove'][] = Array($targetgid, $targetgorder, $depqid, $condid);
            }
        } else { // get all moves constraints
            $resarray['notAbove'][] = Array($targetgid, $targetgorder, $depqid, $condid);
        }
    }
...
function getUserGroupList($ugid=NULL, $outputformat='optionlist') {
    ...
    if (!isset($svexist)) {
        $selecter = "<option value='-1' selected='selected'>" . $clang->gT("Please choose...") . "</option>\n" . $selecter;
    }
...
function doesImportArraySupportLanguage($csvarray, $idkeysarray, $langfieldnum, $langcode, $hasheader = false) {
    ...
    if ($hasheader === true) { // stripping first row to skip headers if any
...
function display_project_status($cur_status_id) {
    $sql = "SELECT * FROM {{project_status_master}} WHERE IsActive=1 and status_for='p' ORDER BY status_order";
    $dr = Yii::app()->db->createCommand($sql)->query(); //Checked
    $html = '<ul class="status" style="margin:0px;">';
    foreach ($dr->readAll() as $r) {
        $cls = "";
        if ($r['status_id'] == $cur_status_id)
            $cls .= "selected";
        $html .= "<li class='$cls' style='background-color:" . $r['status_color'] . ";' title='Status:- " . $r['status_name'] . "'></li>";
    }
    $html .= '</ul>';
    echo $html;
}
